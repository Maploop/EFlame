...
vec4 pointLight()
{
	...
	float diffuse = max(dot(normal, lightDirection), 0.0f); // NOTE: Math.absolute helps with grass shading! But i'm not using it here
    ...
    if (texture(diffuse1, texCoord).a < 0.1)
        discard;
